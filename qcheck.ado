...
************** 4 Default type;
if ("`type'" == "") local type "type(AFRPOV)";
...
dis in yellow "qcheck_static starts";	
	qcheck_static , countries(${qc_countries}) years(${years}) variables(`vari')  sources(${sources}) `vermast' `veralt' `type' module(${module}) `replace' ${survey}  path(`path') outfile("${salt_outfile}") `logfile' test(`test') `fileserver';
